...
CRAQUES_INICIAIS = [
    ...
    (3, "Maracanã",     "França",    "🇫🇷", "#002395", "#ED2939", "⭐"),
    (4, "Pelezão",      "Alemanha",  "🇩🇪", "#000000", "#DD0000", "🦅"),
    (5, "Ronaldão",     "Espanha",   "🇪🇸", "#AA151B", "#F1BF00", "🔥"),
    (6, "Zicozão",      "Portugal",  "🇵🇹", "#006600", "#FF0000", "🦁"),
    (7, "Falcão",       "Holanda",   "🇳🇱", "#FF4F00", "#FFFFFF", "🟠"),
    (8, "Romário",      "Uruguai",   "🇺🇾", "#0038A8", "#FFFFFF", "☀️"),
    (9, "Bebeto",       "Itália",    "🇮🇹", "#009246", "#CE2B37", "🍕"),
]
...
def init_db():
    ...
    if existing == 0:
        c.executemany(
            "INSERT INTO craques (id, nome, selecao, bandeira, cor, cor_sec, emoji, saldo) VALUES (?,?,?,?,?,?,?,?)",
            [(r[0], r[1], r[2], r[3], r[4], r[5], r[6], SALDO_INICIAL) for r in CRAQUES_INICIAIS]
        )
...
def handle_get_craques(handler):
    conn = get_db()
    rows = conn.execute(
        "SELECT id, nome, selecao, bandeira, cor, cor_sec, emoji, saldo FROM craques ORDER BY id"
    ).fetchall()
    conn.close()
...
if __name__ == "__main__":
    ...
    server = HTTPServer(("0.0.0.0", PORT), LeilaoHandler)
