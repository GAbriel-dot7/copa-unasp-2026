...
function renderStats() {
  ...
  set("stat-lider",           `${lider.bandeira} ${lider.nome}`);
}
  ...
  estado.craques.forEach((cr, i) => {
    ...
    card.innerHTML = `
      <div class="card-header">
        <div class="bandeira-circle">${cr.bandeira}</div>
        <div class="card-info">
          <h3 class="craque-nome">${cr.nome}</h3>
          <p class="craque-selecao">${cr.emoji} ${cr.selecao}</p>
        </div>
...
function gerarRelatorio() {
  ...
  t += `  🏆 Mais rico          : ${rico.bandeira} ${rico.nome} (${fmt(rico.saldo)} restante)\n`;
  t += `  🛒 Mais gastou        : ${gastou.bandeira} ${gastou.nome} (${fmt(SALDO_INICIAL - gastou.saldo)} gastos)\n`;
  const njMais = estado.historicoGlobal.filter(h => h.craque_id === maisJog.id).length;
  t += `  📋 Maior elenco       : ${maisJog.bandeira} ${maisJog.nome} (${njMais} jogador${njMais !== 1 ? "es" : ""})\n\n`;
  ...
  ord.forEach((cr, idx) => {
    ...
    t += `${med} ${cr.bandeira} ${cr.nome.toUpperCase()} — ${cr.selecao}\n`;
...
function renderChart() {
  ...
  const labels = estado.craques.map(c => `${c.bandeira} ${c.nome}`);
